...
equivalent per-mesh texture/cbuffer). The verdict surfaces per Shader in
...
decompression failure. `classes` narrows type-tree integers with
...
16-byte-per-pixel stride can overflow into a short allocation. `main`
