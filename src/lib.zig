...
/// Minimal BMP encoder (32bpp BI_BITFIELDS with an alpha mask, top-down).
...
/// re-exported from `container` so name- and byte-based detection stay in
/// one module.
pub const asset_extensions = container.asset_extensions;

/// Returns true when `name` ends in a known Unity asset extension.
pub const isAssetFileName = container.isAssetFileName;
